...
First, clone this repo with the name of the desired provider in place of `oci`:

```
git clone https://github.com/pulumi/pulumi-tf-provider-boilerplate pulumi-oci
```

Second, replace references to `oci` with the name of your provider:
...
- `oci:apiKey` (environment: `XYZ_API_KEY`) - the API key for `oci`
- `oci:region` (environment: `XYZ_REGION`) - the region in which to deploy resources
