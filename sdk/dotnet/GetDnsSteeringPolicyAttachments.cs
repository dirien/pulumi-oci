// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.Oci
{
    public static class GetDnsSteeringPolicyAttachments
    {
        /// <summary>
        /// This data source provides the list of Steering Policy Attachments in Oracle Cloud Infrastructure DNS service.
        /// 
        /// Lists the steering policy attachments in the specified compartment.
        /// 
        /// 
        /// {{% examples %}}
        /// ## Example Usage
        /// {{% example %}}
        /// 
        /// ```csharp
        /// using Pulumi;
        /// using Oci = Pulumi.Oci;
        /// 
        /// class MyStack : Stack
        /// {
        ///     public MyStack()
        ///     {
        ///         var testSteeringPolicyAttachments = Output.Create(Oci.GetDnsSteeringPolicyAttachments.InvokeAsync(new Oci.GetDnsSteeringPolicyAttachmentsArgs
        ///         {
        ///             CompartmentId = @var.Compartment_id,
        ///             DisplayName = @var.Steering_policy_attachment_display_name,
        ///             Domain = @var.Steering_policy_attachment_domain,
        ///             DomainContains = @var.Steering_policy_attachment_domain_contains,
        ///             Id = @var.Steering_policy_attachment_id,
        ///             State = @var.Steering_policy_attachment_state,
        ///             SteeringPolicyId = oci_dns_steering_policy.Test_steering_policy.Id,
        ///             TimeCreatedGreaterThanOrEqualTo = @var.Steering_policy_attachment_time_created_greater_than_or_equal_to,
        ///             TimeCreatedLessThan = @var.Steering_policy_attachment_time_created_less_than,
        ///             ZoneId = oci_dns_zone.Test_zone.Id,
        ///         }));
        ///     }
        /// 
        /// }
        /// ```
        /// {{% /example %}}
        /// {{% /examples %}}
        /// </summary>
        public static Task<GetDnsSteeringPolicyAttachmentsResult> InvokeAsync(GetDnsSteeringPolicyAttachmentsArgs args, InvokeOptions? options = null)
            => Pulumi.Deployment.Instance.InvokeAsync<GetDnsSteeringPolicyAttachmentsResult>("oci:index/getDnsSteeringPolicyAttachments:GetDnsSteeringPolicyAttachments", args ?? new GetDnsSteeringPolicyAttachmentsArgs(), options.WithVersion());
    }


    public sealed class GetDnsSteeringPolicyAttachmentsArgs : Pulumi.InvokeArgs
    {
        /// <summary>
        /// The OCID of the compartment the resource belongs to.
        /// </summary>
        [Input("compartmentId", required: true)]
        public string CompartmentId { get; set; } = null!;

        /// <summary>
        /// The displayName of a resource.
        /// </summary>
        [Input("displayName")]
        public string? DisplayName { get; set; }

        /// <summary>
        /// Search by domain. Will match any record whose domain (case-insensitive) equals the provided value.
        /// </summary>
        [Input("domain")]
        public string? Domain { get; set; }

        /// <summary>
        /// Search by domain. Will match any record whose domain (case-insensitive) contains the provided value.
        /// </summary>
        [Input("domainContains")]
        public string? DomainContains { get; set; }

        [Input("filters")]
        private List<Inputs.GetDnsSteeringPolicyAttachmentsFilterArgs>? _filters;
        public List<Inputs.GetDnsSteeringPolicyAttachmentsFilterArgs> Filters
        {
            get => _filters ?? (_filters = new List<Inputs.GetDnsSteeringPolicyAttachmentsFilterArgs>());
            set => _filters = value;
        }

        /// <summary>
        /// The OCID of a resource.
        /// </summary>
        [Input("id")]
        public string? Id { get; set; }

        /// <summary>
        /// The state of a resource.
        /// </summary>
        [Input("state")]
        public string? State { get; set; }

        /// <summary>
        /// Search by steering policy OCID. Will match any resource whose steering policy ID matches the provided value.
        /// </summary>
        [Input("steeringPolicyId")]
        public string? SteeringPolicyId { get; set; }

        /// <summary>
        /// An [RFC 3339](https://www.ietf.org/rfc/rfc3339.txt) timestamp that states all returned resources were created on or after the indicated time.
        /// </summary>
        [Input("timeCreatedGreaterThanOrEqualTo")]
        public string? TimeCreatedGreaterThanOrEqualTo { get; set; }

        /// <summary>
        /// An [RFC 3339](https://www.ietf.org/rfc/rfc3339.txt) timestamp that states all returned resources were created before the indicated time.
        /// </summary>
        [Input("timeCreatedLessThan")]
        public string? TimeCreatedLessThan { get; set; }

        /// <summary>
        /// Search by zone OCID. Will match any resource whose zone ID matches the provided value.
        /// </summary>
        [Input("zoneId")]
        public string? ZoneId { get; set; }

        public GetDnsSteeringPolicyAttachmentsArgs()
        {
        }
    }


    [OutputType]
    public sealed class GetDnsSteeringPolicyAttachmentsResult
    {
        /// <summary>
        /// The OCID of the compartment containing the steering policy attachment.
        /// </summary>
        public readonly string CompartmentId;
        /// <summary>
        /// A user-friendly name for the steering policy attachment. Does not have to be unique and can be changed. Avoid entering confidential information.
        /// </summary>
        public readonly string? DisplayName;
        public readonly string? Domain;
        public readonly string? DomainContains;
        public readonly ImmutableArray<Outputs.GetDnsSteeringPolicyAttachmentsFilterResult> Filters;
        /// <summary>
        /// The OCID of the resource.
        /// </summary>
        public readonly string? Id;
        /// <summary>
        /// The current state of the resource.
        /// </summary>
        public readonly string? State;
        /// <summary>
        /// The list of steering_policy_attachments.
        /// </summary>
        public readonly ImmutableArray<Outputs.GetDnsSteeringPolicyAttachmentsSteeringPolicyAttachmentResult> SteeringPolicyAttachments;
        /// <summary>
        /// The OCID of the attached steering policy.
        /// </summary>
        public readonly string? SteeringPolicyId;
        public readonly string? TimeCreatedGreaterThanOrEqualTo;
        public readonly string? TimeCreatedLessThan;
        /// <summary>
        /// The OCID of the attached zone.
        /// </summary>
        public readonly string? ZoneId;

        [OutputConstructor]
        private GetDnsSteeringPolicyAttachmentsResult(
            string compartmentId,

            string? displayName,

            string? domain,

            string? domainContains,

            ImmutableArray<Outputs.GetDnsSteeringPolicyAttachmentsFilterResult> filters,

            string? id,

            string? state,

            ImmutableArray<Outputs.GetDnsSteeringPolicyAttachmentsSteeringPolicyAttachmentResult> steeringPolicyAttachments,

            string? steeringPolicyId,

            string? timeCreatedGreaterThanOrEqualTo,

            string? timeCreatedLessThan,

            string? zoneId)
        {
            CompartmentId = compartmentId;
            DisplayName = displayName;
            Domain = domain;
            DomainContains = domainContains;
            Filters = filters;
            Id = id;
            State = state;
            SteeringPolicyAttachments = steeringPolicyAttachments;
            SteeringPolicyId = steeringPolicyId;
            TimeCreatedGreaterThanOrEqualTo = timeCreatedGreaterThanOrEqualTo;
            TimeCreatedLessThan = timeCreatedLessThan;
            ZoneId = zoneId;
        }
    }
}
