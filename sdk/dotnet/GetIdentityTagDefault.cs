// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.Oci
{
    public static class GetIdentityTagDefault
    {
        /// <summary>
        /// This data source provides details about a specific Tag Default resource in Oracle Cloud Infrastructure Identity service.
        /// 
        /// Retrieves the specified tag default.
        /// 
        /// 
        /// {{% examples %}}
        /// ## Example Usage
        /// {{% example %}}
        /// 
        /// ```csharp
        /// using Pulumi;
        /// using Oci = Pulumi.Oci;
        /// 
        /// class MyStack : Stack
        /// {
        ///     public MyStack()
        ///     {
        ///         var testTagDefault = Output.Create(Oci.GetIdentityTagDefault.InvokeAsync(new Oci.GetIdentityTagDefaultArgs
        ///         {
        ///             TagDefaultId = oci_identity_tag_default.Test_tag_default.Id,
        ///         }));
        ///     }
        /// 
        /// }
        /// ```
        /// {{% /example %}}
        /// {{% /examples %}}
        /// </summary>
        public static Task<GetIdentityTagDefaultResult> InvokeAsync(GetIdentityTagDefaultArgs args, InvokeOptions? options = null)
            => Pulumi.Deployment.Instance.InvokeAsync<GetIdentityTagDefaultResult>("oci:index/getIdentityTagDefault:GetIdentityTagDefault", args ?? new GetIdentityTagDefaultArgs(), options.WithVersion());
    }


    public sealed class GetIdentityTagDefaultArgs : Pulumi.InvokeArgs
    {
        /// <summary>
        /// The OCID of the tag default.
        /// </summary>
        [Input("tagDefaultId", required: true)]
        public string TagDefaultId { get; set; } = null!;

        public GetIdentityTagDefaultArgs()
        {
        }
    }


    [OutputType]
    public sealed class GetIdentityTagDefaultResult
    {
        /// <summary>
        /// The OCID of the compartment. The tag default applies to all new resources that get created in the compartment. Resources that existed before the tag default was created are not tagged.
        /// </summary>
        public readonly string CompartmentId;
        /// <summary>
        /// The OCID of the tag default.
        /// </summary>
        public readonly string Id;
        /// <summary>
        /// If you specify that a value is required, a value is set during resource creation (either by the user creating the resource or another tag defualt). If no value is set, resource creation is blocked.
        /// * If the `isRequired` flag is set to "true", the value is set during resource creation.
        /// * If the `isRequired` flag is set to "false", the value you enter is set during resource creation.
        /// </summary>
        public readonly bool IsRequired;
        /// <summary>
        /// The tag default's current state. After creating a `TagDefault`, make sure its `lifecycleState` is ACTIVE before using it.
        /// </summary>
        public readonly string State;
        public readonly string TagDefaultId;
        /// <summary>
        /// The OCID of the tag definition. The tag default will always assign a default value for this tag definition.
        /// </summary>
        public readonly string TagDefinitionId;
        /// <summary>
        /// The name used in the tag definition. This field is informational in the context of the tag default.
        /// </summary>
        public readonly string TagDefinitionName;
        /// <summary>
        /// The OCID of the tag namespace that contains the tag definition.
        /// </summary>
        public readonly string TagNamespaceId;
        /// <summary>
        /// Date and time the `TagDefault` object was created, in the format defined by RFC3339.  Example: `2016-08-25T21:10:29.600Z`
        /// </summary>
        public readonly string TimeCreated;
        /// <summary>
        /// The default value for the tag definition. This will be applied to all new resources created in the compartment.
        /// </summary>
        public readonly string Value;

        [OutputConstructor]
        private GetIdentityTagDefaultResult(
            string compartmentId,

            string id,

            bool isRequired,

            string state,

            string tagDefaultId,

            string tagDefinitionId,

            string tagDefinitionName,

            string tagNamespaceId,

            string timeCreated,

            string value)
        {
            CompartmentId = compartmentId;
            Id = id;
            IsRequired = isRequired;
            State = state;
            TagDefaultId = tagDefaultId;
            TagDefinitionId = tagDefinitionId;
            TagDefinitionName = tagDefinitionName;
            TagNamespaceId = tagNamespaceId;
            TimeCreated = timeCreated;
            Value = value;
        }
    }
}
