// *** WARNING: this file was generated by the Pulumi Terraform Bridge (tfgen) Tool. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.Oci
{
    public static class GetApmSyntheticsResult
    {
        /// <summary>
        /// This data source provides details about a specific Result resource in Oracle Cloud Infrastructure Apm Synthetics service.
        /// 
        /// Gets the results for a specific execution of a monitor identified by OCID. The results are in a HAR file, Screenshot, or Console Log.
        /// 
        /// 
        /// {{% examples %}}
        /// ## Example Usage
        /// {{% example %}}
        /// 
        /// ```csharp
        /// using Pulumi;
        /// using Oci = Pulumi.Oci;
        /// 
        /// class MyStack : Stack
        /// {
        ///     public MyStack()
        ///     {
        ///         var testResult = Output.Create(Oci.GetApmSyntheticsResult.InvokeAsync(new Oci.GetApmSyntheticsResultArgs
        ///         {
        ///             ApmDomainId = oci_apm_synthetics_apm_domain.Test_apm_domain.Id,
        ///             ExecutionTime = @var.Result_execution_time,
        ///             MonitorId = oci_apm_synthetics_monitor.Test_monitor.Id,
        ///             ResultContentType = @var.Result_result_content_type,
        ///             ResultType = @var.Result_result_type,
        ///             VantagePoint = @var.Result_vantage_point,
        ///         }));
        ///     }
        /// 
        /// }
        /// ```
        /// {{% /example %}}
        /// {{% /examples %}}
        /// </summary>
        public static Task<GetApmSyntheticsResultResult> InvokeAsync(GetApmSyntheticsResultArgs args, InvokeOptions? options = null)
            => Pulumi.Deployment.Instance.InvokeAsync<GetApmSyntheticsResultResult>("oci:index/getApmSyntheticsResult:GetApmSyntheticsResult", args ?? new GetApmSyntheticsResultArgs(), options.WithVersion());
    }


    public sealed class GetApmSyntheticsResultArgs : Pulumi.InvokeArgs
    {
        /// <summary>
        /// The APM domain ID the request is intended for.
        /// </summary>
        [Input("apmDomainId", required: true)]
        public string ApmDomainId { get; set; } = null!;

        /// <summary>
        /// The time the object was posted.
        /// </summary>
        [Input("executionTime", required: true)]
        public string ExecutionTime { get; set; } = null!;

        /// <summary>
        /// The OCID of the monitor.
        /// </summary>
        [Input("monitorId", required: true)]
        public string MonitorId { get; set; } = null!;

        /// <summary>
        /// The result content type zip or raw.
        /// </summary>
        [Input("resultContentType", required: true)]
        public string ResultContentType { get; set; } = null!;

        /// <summary>
        /// The result type har or screenshot or log.
        /// </summary>
        [Input("resultType", required: true)]
        public string ResultType { get; set; } = null!;

        /// <summary>
        /// The vantagePoint name.
        /// </summary>
        [Input("vantagePoint", required: true)]
        public string VantagePoint { get; set; } = null!;

        public GetApmSyntheticsResultArgs()
        {
        }
    }


    [OutputType]
    public sealed class GetApmSyntheticsResultResult
    {
        public readonly string ApmDomainId;
        /// <summary>
        /// The specific point of time when the result of an execution is collected.
        /// </summary>
        public readonly string ExecutionTime;
        /// <summary>
        /// The provider-assigned unique ID for this managed resource.
        /// </summary>
        public readonly string Id;
        /// <summary>
        /// The [OCID](https://docs.cloud.oracle.com/iaas/Content/General/Concepts/identifiers.htm) of the monitor.
        /// </summary>
        public readonly string MonitorId;
        /// <summary>
        /// Type of result content. Example: Zip or Raw file.
        /// </summary>
        public readonly string ResultContentType;
        /// <summary>
        /// Monitor result data set.
        /// </summary>
        public readonly ImmutableArray<Outputs.GetApmSyntheticsResultResultDataSetResult> ResultDataSets;
        /// <summary>
        /// Type of result. Example: HAR, Screenshot or Log.
        /// </summary>
        public readonly string ResultType;
        /// <summary>
        /// The name of the vantage point.
        /// </summary>
        public readonly string VantagePoint;

        [OutputConstructor]
        private GetApmSyntheticsResultResult(
            string apmDomainId,

            string executionTime,

            string id,

            string monitorId,

            string resultContentType,

            ImmutableArray<Outputs.GetApmSyntheticsResultResultDataSetResult> resultDataSets,

            string resultType,

            string vantagePoint)
        {
            ApmDomainId = apmDomainId;
            ExecutionTime = executionTime;
            Id = id;
            MonitorId = monitorId;
            ResultContentType = resultContentType;
            ResultDataSets = resultDataSets;
            ResultType = resultType;
            VantagePoint = vantagePoint;
        }
    }
}
